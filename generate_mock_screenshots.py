...
def create_main_screenshot():
    ...
    draw.text((100, 22), "Баланс: 1000 грн", fill="#2e7d32", anchor="lm", font=get_font(10, True))
    
    # Кнопка поповнення балансу
    draw.rounded_rectangle([210, 10, 300, 35], 3, fill="#2ecc71")
    draw.text((255, 22), "+ Поповнити", fill="#ffffff", anchor="mm", font=get_font(8, True))
    ...
    # Заголовок
    draw.text((w//2, 125), "Фруктовий Маркет", fill="#212529", anchor="mm", font=get_font(22, True))
    ...
    # Категорії
    categories = ["Усі", "Ягоди", "Цитруси", "Інші"]
    ...
    # Карточки
    fruits = [
        ("Яблуко", "20 грн/шт", "apple.png", True),
        ("Банан", "15 грн/шт", "banana.png", False),
        ("Апельсин", "25 грн/шт", "orange.png", False),
        ("Полуниця", "50 грн/шт", "strawberry.png", True),
        ("Виноград", "40 грн/шт", "grapes.png", False),
        ("Кавун", "100 грн/шт", "watermelon.png", False)
    ]
    
    # Сортуємо так, щоб обрані (True) йшли спочатку
    fruits.sort(key=lambda x: 0 if x[3] else 1)
    
    for i, (name, price, img_file, is_fav) in enumerate(fruits):
        row = i // 3
        col = i % 3
        cx = 50 + col * 175
        cy = 280 + row * 165
        ...
        if os.path.exists(img_file):
            fruit_icon = Image.open(img_file).resize((40, 40))
            img.paste(fruit_icon, (cx + 55, cy + 20), fruit_icon.convert("RGBA"))
            
        draw.text((cx + 75, cy + 80), name, fill="#212529", anchor="mm", font=get_font(12, True))
...
def create_details_screenshot():
    ...
    if os.path.exists("apple.png"):
        fruit_icon = Image.open("apple.png").resize((80, 80))
        img.paste(fruit_icon, (w//2 - 40, 20), fruit_icon.convert("RGBA"))
        
    draw.text((w//2, 120), "Яблуко", fill="#212529", anchor="mm", font=get_font(18, True))
    draw.text((w//2, 145), "Соковите червоне або зелене яблуко", fill="#6c757d", anchor="mm", font=get_font(10))
    draw.text((w//2, 170), "Ціна: 20 грн/шт", fill="#2e7d32", anchor="mm", font=get_font(12, True))
    ...
    draw.text((w//2 + 35, 280), "2", fill="#212529", anchor="mm", font=get_font(11))
    ...
    draw.text((20, 415), "Рейтинг: ★★★★☆ (4.0/5)", fill="#f1c40f", anchor="lm", font=get_font(10, True))
    draw.text((20, 440), "• Yarik (5★): Дуже смачні та свіжі яблука!", fill="#555555", anchor="lm", font=get_font(9))
    draw.text((20, 460), "• Guest (3★): Звичайні яблука, нічого особливого.", fill="#555555", anchor="lm", font=get_font(9))
    
    # Додати відгук
    draw.text((20, 490), "Додати відгук: [ 5 ★ ] [ Текст відгуку... ] [ Надіслати ]", fill="#888888", anchor="lm", font=get_font(9, True))
...
def create_cart_screenshot():
    ...
    draw.text((30, 80), "Яблуко (Червоне 🔴) x2", fill="#212529", anchor="lm", font=get_font(11, True))
    draw.text((250, 80), "40 грн", fill="#555555", anchor="lm", font=get_font(11))
    ...
    draw.text((30, 130), "Банан (Жовтий 🟡) x3", fill="#212529", anchor="lm", font=get_font(11, True))
    draw.text((250, 130), "45 грн", fill="#555555", anchor="lm", font=get_font(11))
    draw.rectangle([w - 45, 120, w - 25, 140], fill="#ff4d4d")
    draw.text((w - 35, 130), "❌", fill="#ffffff", anchor="mm", font=get_font(9, True))
    
    # Розрахунок зі знижкою 15% від Колеса Фортуни
    draw.text((w//2, 220), "Сума: 85 грн", fill="#555555", anchor="mm", font=get_font(11))
    draw.text((w//2, 245), "Знижка (15%): -12 грн", fill="#ff4d4d", anchor="mm", font=get_font(11))
    draw.text((w//2, 280), "Разом до сплати: 73 грн", fill="#2e7d32", anchor="mm", font=get_font(14, True))
...
if __name__ == "__main__":
    ...
    print("All upgraded mock screenshots successfully generated with system Cyrillic fonts (v3)!")
