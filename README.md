...
```mermaid
graph TD
    START(["▶️ SilpoMarket.exe"]) --> SESSION

    subgraph SESSION ["💾 Перевірка сесії"]
        S1{"session.txt\nіснує?"} -->|Так| S2["✅ Авто-вхід"]
        S1 -->|Ні| AUTH
    end

    subgraph AUTH ["🔐 Авторизація"]
        A1["📋 Форма входу/реєстрації"]
        A2{"Новий\nкористувач?"}
        A3["➕ Реєстрація\nSHA-256 пароль"]
        A4["🔑 Перевірка\nу SQLite"]
        A1 --> A2
    ...
    subgraph CHECKOUT ["💳 Оформлення замовлення"]
        C1["⚡ Автозаповнення форми"]
        C2["🚚 Доставка"]
        C3{"Спосіб\nоплати?"}
        C4["💳 Балансом\nакаунту"]
        C5["💳 Карткою\nпри отриманні"]
    ...
    C4 -->|Знімає баланс| DB
    C5 -->|Форма картки| CARD
    C6 -->|Одразу| RECEIPT

    subgraph CARD ["💳 Введення картки"]
        K1["⚡ Автозаповнення"]
        K2["🔒 Дані картки"]
        K1 --> K2
    end
    ...
    THREADS["🔄 Фонові потоки\nthreading"] -->|urllib| CACHE["📥 Фото товарів\ncache_images/"]
    M1 --> CACHE
```
...
|:----------:|:------:|:-----------:|:-----------:|
| ![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white) | 3.8+ | Основна мова | Простий синтаксис, велика екосистема |
| ![CustomTkinter](https://img.shields.io/badge/CustomTkinter-FF6B6B?logo=python&logoColor=white) | Latest | GUI фреймворк | Сучасний вигляд, підтримка тем |
| ![SQLite](https://img.shields.io/badge/SQLite3-07405E?logo=sqlite&logoColor=white) | Built-in | База даних | Вбудована в Python, без сервера |
| ![Pillow](https://img.shields.io/badge/Pillow-F5A623?logo=python&logoColor=white) | Latest | Зображення | Завантаження та масштабування PNG |
| ![PyInstaller](https://img.shields.io/badge/PyInstaller-3776AB?logo=python&logoColor=white) | 6.x | Компіляція EXE | Один файл без залежностей |
| ![Threading](https://img.shields.io/badge/threading-lightgrey?logo=python&logoColor=black) | Built-in | Фоновий завантажувач | Без зависання GUI |
| ![urllib](https://img.shields.io/badge/urllib-lightblue?logo=python&logoColor=black) | Built-in | HTTP запити | Завантаження фото товарів |
...
gitGraph LR:
   commit id: "🚀 Init"
   commit id: "📦 504 товари"
   commit id: "🔐 Авторизація"
   commit id: "🎨 Теми UI" tag: "v1.0.0"
   commit id: "💰 Гаманець"
   commit id: "💳 Оплата картою" tag: "v1.1.0"
   commit id: "🛒 3 способи оплати"
   commit id: "⚡ Автозаповнення форм"
   commit id: "🧾 Преміум чеки"
   commit id: "📋 requirements.txt" tag: "v1.2.0"
```
...
[![Release](https://img.shields.io/badge/Download-v1.2.0-2ECC71?style=for-the-badge&logo=github)](https://github.com/greenyarik0505-jpg/Privet/releases/latest)
